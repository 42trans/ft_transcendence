...
class TestGanacheSave(TestCase):
	...
	def test_save_game_result_success(self):
		...
		response = self.client.post(reverse('save_testnet', args=['ganache']), json.dumps(self.data), content_type='application/json')
	...
	def test_save_game_result_bad_request(self):
		"""不正なデータでリクエストを送る"""
		response = self.client.post(reverse('save_testnet', args=['ganache']), '{}', content_type='application/json')
		self.assertEqual(response.status_code, 400)  
		
	def test_save_game_result_invalid_method(self):
		"""不正なHTTPメソッド(GET)でリクエストを送る"""
		response = self.client.put(reverse('save_testnet', args=['ganache']))
		self.assertEqual(response.status_code, 400) 

	def test_save_game_result_invalid_json(self):
		"""不正なJSON形式でリクエストを送る"""
		response = self.client.post(reverse('save_testnet', args=['ganache']), '{bad json', content_type='application/json')
		self.assertEqual(response.status_code, 400)
