...
class PongOnlineDuelReceiveHandler:
    ...
    # Redisの混乱するポイント
        ...
        - 値はリストやセットなどのデータ構造も可能。実質dict,mapなのに名前はset
    ...
    async def handle_start_action(self, json_data):
        ...
        Websocketの状態で2名の状態を判断したい。そのためにRedisを使用する。

        ## Redis key  f"start_signals_* に関する処理の流れ: RedisのsetがC++とちょっと違う(setを値とするmap) : 
            - 使用するRedisのセット: f"start_signals_{self.game_manager.room_group_name}" : クライアントがstartボタンを押した数を数える目的。このメソッド内ですぐに削除する
            - キーの生成(sadd): f"start_signals_{self.game_manager.room_group_name}" がない場合、ゲームルームに固有のキーを生成。
        ...
        if ASYNC_LOG_FOR_DEV:
            await async_log("開始:handle_start_action()")
        try:
    ...
    async def handle_update_action(self, json_data):
        ...
        """
        # if ASYNC_LOG_FOR_DEV:
        #     await async_log("開始:handle_update_action()")
        ...
        # async with g_REDIS_STATE_LOCK:
        #     redis_client = self.resources.get_redis_client()
        #     game_state_json = await database_sync_to_async(redis_client.get)(
        #         f"game_state:{self.consumer.room_group_name}"
        #     )
        #     if game_state_json:
    ...
    async def handle_invalid_action(self):
        """ 
        期待されるキーが含まれていない場合
